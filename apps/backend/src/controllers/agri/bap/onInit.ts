...
import { AGRI_EXAMPLES_PATH, SERVICES_EXAMPLES_PATH, checkIfCustomized, quoteCreatorHealthCareService, redisFetchFromServer, responseBuilder } from "../../../lib/utils";
...
const onInitConsultationController = (req: Request, res: Response, next: NextFunction) => {
	try{
		...
		const file = fs.readFileSync(
			path.join(AGRI_EXAMPLES_PATH, "confirm/confirm.yaml")
		);
