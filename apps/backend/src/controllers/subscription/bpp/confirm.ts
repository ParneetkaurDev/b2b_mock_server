...
export const confirmConsultationController = async (
	...
	try {
		...
		if (scenario && scenario !== "single-order-offline-without-subscription" && scenario && scenario !== "single-order-online-without-subscription") {
			...
			let interval = setInterval(() => {
				...
				responseMessage.order.status = "Active";
				childOrderResponseBuilder(
					i,
					res,
					context,
