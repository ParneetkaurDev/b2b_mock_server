...
export const responseBuilder = async (
	...
	console.log("mode jss",res.getHeader("mode"))
	...
	const bppURI =
		domain === "b2b"
			...
			console.log("contxt",reqContext,"action",action,"bppURI",bppURI)
	...
	console.log("responseBuilderHeader",header)
	if (sandboxMode) {
		if (action.startsWith("on_")) {
			...
			try {
				console.log(`uri at responseBUilder ${uri},async :${async}`)
				const response = await axios.post(`${uri}?mode=mock`, async, {
					headers: {
						authorization: header,
					},
				});
				console.log("Response at responseBUilder",response.data)
				log.response = {
		...
		console.log("responseBUilder ack1")
		...
		console.log("responseBUilder ack2")
		return res.json({
			sync: {
				message: {
					ack: {
