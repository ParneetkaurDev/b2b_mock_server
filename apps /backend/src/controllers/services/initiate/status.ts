...
const intializeRequest = async (
	...
	try {
		...
		console.log("initate domain",domain)
		switch (domain) {
			case SERVICES_DOMAINS.SERVICES:
				...
				senarios = EQUIPMENT_HIRING_STATUS;
				break;
			case SERVICES_DOMAINS.BID_ACTION_SERVICES://srv18
		...
		console.log("initate statusIndex",statusIndex)
		await send_response(
			res,
			next,
			status,
		...
		console.log("at InitateRequest status aftre sendResponse")

		
		// try{
		// 	 await axios.post("http://localhost:3005/api/services/initiate/update",
		// 	)
		...
		// 	console.log(err)
		// }
		// setTimeout(async()=>{await send_response(
		// 	res,
		// 	next,
		// 	status,
		// 	transaction_id,
