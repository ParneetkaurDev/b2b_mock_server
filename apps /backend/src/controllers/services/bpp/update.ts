...
export const UpdateControllerdemo=async(
	req:Request,
	res:Response,
	next:NextFunction
)=>{
	console.log("updateControllerDemo")
	try{
		const on_status = await redisFetchToServer(
			ON_ACTION_KEY.ON_STATUS,
			req.body.context.transaction_id
		);
		if (!on_status) {
			return send_nack(res, ERROR_MESSAGES.ON_STATUS_DOES_NOT_EXISTED);
		}
...
export const updateRequoteController = (
	...
	try {
		...
		//CREATED COMMON RESPONSE MESSAGE FOR ALL SCENRIO AND UPDATE ACCORDENGLY IN FUNCTIONS
			console.log("hello from updatrRequoteCOntroller")
